...
//
//// Derived sizes
//const PK_BYTES: usize = 8 * G1_BYTES + 2 * GT_BYTES;
//const SK_BYTES: usize = 15 * SCALAR_BYTES;
//const USK_BYTES: usize = 6 * G2_BYTES;
//const CT_BYTES: usize = 5 * G1_BYTES + 1 * GT_BYTES;
...
pub fn extract_usk<R: Rng>(sk: &SecretKey, v: &Identity, rng: &mut R) -> UserSecretKey {
    ...
    let xbrminusk = [
        alpha2 - sk.k[0] - (x[0][0] * br[0] + x[0][1] * br[1]),
        alpha2 - sk.k[1] - (x[1][0] * br[0] + x[1][1] * br[1]),
    ];
...
pub fn encaps<R: Rng>(pk: &PublicKey, id: &Identity, rng: &mut R) -> (CipherText, SharedSecret) {
    ...
    // TODO: optimization: use batch_normalize here too

    let c0 = [(pk.a_1[0] * s).into(), (pk.a_1[1] * s).into()];
    let y = hash_g1_to_scalar(c0[0]);
    let cprimeprime: G1Affine = ((pk.wprime_1[0] * s) + (pk.wprime_1[1]) * (s * y)).into();
...
pub fn decaps(usk: &UserSecretKey, ct: &CipherText) -> SharedSecret {
    ...
    let z: G2Affine = (usk.kprime[0] + (usk.kprime[1] * y)).into();
    ...
    let k = ct.cprime
        + multi_miller_loop(&[
            ...
            (&ct.cprimeprime, &G2Prepared::from(usk.d0[0])),
        ])
